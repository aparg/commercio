...
const MapInfoCard = ({ property }) => {
  ...
  const slug = `${property.StreetNumber}-${property.StreetName?.replace(
    /\s+/g,
    "-"
  )?.toLowerCase()}`;
  const href = `/commercial/ontario/${property.City?.toLowerCase()}/homes-for-sale/${slug}`;
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-sm">
          ...
          <div className="text-center p-2 bg-gray-50 rounded">
            <p className="font-semibold">
              {property.LivingArea
                ? `${Math.round(property.LivingArea)}`
                : "N/A"}
            </p>
