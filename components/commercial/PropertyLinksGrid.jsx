...
const PropertyLinksGrid = ({ currentCity = "Toronto" }) => {
  ...
  const propertyTypes = [
    { label: "Retail", slug: "retail", suffix: "" },
    { label: "Industrial", slug: "industrial-space", suffix: "" },
    { label: "Land", slug: "land", suffix: "" },
    { label: "Office", slug: "office", suffix: "" },
  ];
  ...
  const generatePropertyTypeURL = (city, propertyType) => {
    return `/commercial/ontario/${city.toLowerCase()}/${propertyType}-for-sale`;
  };

  const generatePriceRangeURL = (city, priceRange) => {
    return `/commercial/ontario/${city.toLowerCase()}/${priceRange}-for-sale`;
  };

  const generateOpenHouseURL = (city) => {
    return `/commercial/ontario/${city.toLowerCase()}/open-houses`;
  };

  const generatePriceDroppedURL = (city) => {
    return `/commercial/ontario/${city.toLowerCase()}/price-dropped`;
  };

  const generateBedBathURL = (city, slug) => {
    return `/commercial/ontario/${city.toLowerCase()}/homes-for-sale/${slug}`;
  };
    ...
    <div className="rounded-xl">
      <div className="mb-8">
        ...
        <p className="text-black">
          Explore commercial spaces for sale in {city || "Ontario"}. Price
          Dropped Listings Available.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-x-12 gap-y-8">
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-800 mb-3">
            {city} Commercial Spaces by Property Type
          </h3>
        ...
        <div>
          <h3 className="text-lg font-semibold text-gray-800 mb-3">
            {city} Commercial Spaces by Price Range
          </h3>
          <ul className="flex flex-col space-y-1.5 list-none p-0 m-0">
  ...
  const OtherCityLinks = ({ city }) => (
    <div className="flex flex-col space-y-3">
      <h4 className="text-lg font-bold text-gray-800">
        <Link
          href={`/commercial/ontario/${city.toLowerCase()}/homes-for-sale`}
