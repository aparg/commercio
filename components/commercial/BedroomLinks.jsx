...
const BedroomLinks = ({ propertyType, saleLease }) => {
  ...
  const generateBedroomURL = ({ cityVal, propertyType, bedCount }) => {
    if (propertyType == "Homes") {
      return `/commercial/ontario/${cityVal.toLowerCase()}/homes-for-sale/${bedCount}-plus-bed`;
    } else {
      return `/commercial/ontario/${cityVal.toLowerCase()}/${propertyType.toLowerCase()}-homes-for-sale/${bedCount}-plus-bed`;
    }
  };
